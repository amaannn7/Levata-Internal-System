...
2. **Documents** — **Cost Proposals** (placeholder page; generator pending the template) and **Statements of Work** (the existing Document Studio / SOW generator).
...
### SOW status / TODO
- Done & lint-clean: generate, refine, Fireflies pull, transcript extraction, sample data, PDF export.
- Remaining: test end-to-end with a real AI key (Gemini); tune PDF fidelity vs the reference SOW; deploy to the live app.

## Levata rebrand (de-Macktiles) — DONE

The whole app was rebranded from Macktiles (tiles) to Levata, brand + logic:
- **Brand:** `logo.png` / `logo-inverse.png` are now the Levata logo (`levatalogo.png`). The sidebar/mobile logo uses CSS `filter: brightness(0) invert(1)` because the Levata logo is dark-on-transparent on a dark surface (a dedicated white logo would be cleaner). Title/login/sidebar/onboarding text say "Levata". CSS tokens: `--navy` is deep slate `#1c1530`, `--gold` is Levata violet `#7c3aed` (was Macktiles terracotta).
...
- **Left as-is (internal, not user-facing):** Zoho-mapping function names, `getMacktilesStages`, `macktilesCallOutcomeConfig`, the CORS origin `sales.macktiles.com.au` (set to Levata's real domain at deploy), and an inert Melbourne scoring fallback.
- **When deploying:** update the CORS `$allowedOrigins` in `api.php` to Levata's domain; consider a real white logo for the dark sidebar.
